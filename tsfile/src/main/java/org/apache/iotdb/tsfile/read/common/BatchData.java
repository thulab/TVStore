...
public class BatchData implements Serializable {
  ...
  public void putAnBatchData(BatchData batchData, int len) {
    switch (dataType) {
      case BOOLEAN:
        for(int i=0;i<len;i++){
          putTime(batchData.getTimeByIndex(i));
          putBoolean(batchData.getBooleanByIndex(i));
        }
        break;
      case INT32:
        for(int i=0;i<len;i++){
          putTime(batchData.getTimeByIndex(i));
          putInt(batchData.getIntByIndex(i));
        }
        break;
      case INT64:
        for(int i=0;i<len;i++){
          putTime(batchData.getTimeByIndex(i));
          putLong(batchData.getLongByIndex(i));
        }
        break;
      case FLOAT:
        for(int i=0;i<len;i++){
          putTime(batchData.getTimeByIndex(i));
          putFloat(batchData.getFloatByIndex(i));
        }
        break;
      case DOUBLE:
        for(int i=0;i<len;i++){
          putTime(batchData.getTimeByIndex(i));
          putDouble(batchData.getDoubleByIndex(i));
        }
        break;
      case TEXT:
        for(int i=0;i<len;i++){
          putTime(batchData.getTimeByIndex(i));
          putBinary(batchData.getBinaryByIndex(i));
        }
        break;
      default:
        throw new UnSupportedDataTypeException(String.valueOf(dataType));
  ...
  public void putAnBatchData(BatchData batchData) {
    int len = batchData.length();
    putAnBatchData(batchData, len);
  }
