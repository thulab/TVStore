...
public class TsFileIOWriter {
  ...
  // 实际的page数目为：
  protected int actualPageNum = 0;
  // 假设不压缩则page数目为：
  protected int virtuaPageNum = 0;
  ...
  public void writeChunk(Chunk chunk, ChunkMetaData chunkMetadata) throws IOException {
    ...
    // FIXME merge 于 compress 过程有冲突，未来考虑在chunkMeta里是否添加这两个字段，或者更改最后更新resource的过程
    endChunk(chunkMetadata.getNumOfPoints(), 0, 0);
  }
   ...
   */
  public void endChunk(long totalValueCount, int actualPageNum, int virtualPageNum) {
    ...
    totalChunkNum++;
    this.virtuaPageNum+=virtualPageNum;
  ...
  public int getVirtuaPageNum() {
    return virtuaPageNum;
  }
