...
public class DoubleStatistics extends Statistics<Double> {
  ...
  public void updateStats(double value) {
    if (this.isEmpty) {
      initializeStats(value, value, value, value, value, 1);
      isEmpty = false;
    } else {
      updateStats(value, value, value, value, value, 1);
    }
  }
  ...
  public void updateStats(double[] values, int batchSize) {
    for (int i = 0; i < batchSize; i++) {
      if (isEmpty) {
        initializeStats(values[i], values[i], values[i], values[i], values[i], 1);
        isEmpty = false;
      } else {
        updateStats(values[i], values[i], values[i], values[i], values[i], 1);
      }
    }
  }

  private void updateStats(double minValue, double maxValue, double firstValue, double lastValue,
      double sumValue, long countValue) {
    if (minValue < min) {
      min = minValue;
    }
    if (maxValue > max) {
  ...
  protected void mergeStatisticsValue(Statistics<?> stats) {
    ...
    if (this.isEmpty) {
      initializeStats(doubleStats.getMin(), doubleStats.getMax(), doubleStats.getFirst(),
          doubleStats.getLast(), doubleStats.getSum(), doubleStats.getCount());
      isEmpty = false;
    } else {
   ...
   * @param sum sum value
   */
  private void initializeStats(double min, double max, double first, double last, double sum, long count) {
    this.min = min;
    this.max = max;
    this.first = first;
    this.last = last;
  ...
  void deserialize(InputStream inputStream) throws IOException {
    ...
    //this.count = ReadWriteIOUtils.readLong(inputStream);
  }
