...
public class StorageEngine implements IService {
  ...
  private long lastRecordWritePointNumTime;
  private double lastIngestRate = -1;
  private double lastCompressRate = config.getDiskReadSpeed();
  private double curCompressRate = 0;
  private int compressStorageGroupNum = 0;
  ...
  private ScheduledExecutorService writeSpeedStatisticThread;
  ...
  private StorageEngine() {
    ...
    lastRecordWritePointNumTime = System.currentTimeMillis();
  }
  ...
  public void start() {
    ...
    writeSpeedStatisticThread = Executors.newSingleThreadScheduledExecutor();
    writeSpeedStatisticThread.scheduleAtFixedRate(this::updateIngestSpeed, TTL_CHECK_INTERVAL,
        TTL_CHECK_INTERVAL, TimeUnit.MILLISECONDS);
  }
  ...
  private void updateIngestSpeed() {
    long totalNum = 0;
    for (StorageGroupProcessor processor : processorMap.values()) {
      totalNum += processor.updateIngestSpeed();
    }
    double curSpeed = 1.0 * totalNum / (System.currentTimeMillis() - lastRecordWritePointNumTime)/1000;

    if (lastIngestRate <= 0) {
      lastIngestRate = curSpeed;
    } else {
      lastIngestRate = (lastIngestRate + curSpeed) / 2;
    }
    lastRecordWritePointNumTime = System.currentTimeMillis();
  }

  public void updateCompressSpeed(double speed){
    // curCompressRate
    if(curCompressRate == 0 || curCompressRate>speed){
      curCompressRate = speed;
    }
    compressStorageGroupNum--;

    if(compressStorageGroupNum == 0){
      lastCompressRate = (lastCompressRate+curCompressRate)/2;
      curCompressRate = 0;
    }
  }
  ...
  /**
   * compress all storage groups.
   *
   * @throws StorageEngineException StorageEngineException
   */
  public void compressAll() throws StorageEngineException {
    if (IoTDBDescriptor.getInstance().getConfig().isReadOnly()) {
      throw new StorageEngineException(
          "Current system mode is read only, does not support compress");
    }


    int unFinishCompressNum=0;
    for (Entry<String, StorageGroupProcessor> entry : processorMap.entrySet()) {
      if(entry.getValue().isCompressing()){
        unFinishCompressNum++;
      }
    }

    if(unFinishCompressNum>0){
      return;
    }

    String[] uncompressStorageGroupArray = config.getUncompressStorageGroupList().split(",");

    Map<String, Double> storageGroupCompress = new HashMap<>();
    double totalRatio = 0;
    for (Entry<String, StorageGroupProcessor> entry : processorMap.entrySet()) {
      StorageGroupProcessor storageGroupProcessor = entry.getValue();
      if(storageGroupProcessor.isCompressing()){
        continue;
      }
      for(String unCompressStrorageGroup:uncompressStorageGroupArray){
        if(entry.getKey().equalsIgnoreCase(unCompressStrorageGroup)){
          continue;
        }
      }

      storageGroupCompress.put(entry.getKey(), storageGroupProcessor.calCompressRatio());
      totalRatio+=storageGroupCompress.get(entry.getKey());
    }

    double avgRatio = totalRatio/storageGroupCompress.size();
    double threshold = 2*config.getCompressionRatioDiff()/(1+config.getCompressionRatioDiff());
    for (Entry<String, Double> entry : storageGroupCompress.entrySet()) {
      StorageGroupProcessor storageGroupProcessor = processorMap.get(entry.getKey());
      if(storageGroupProcessor.isCompressing() || entry.getValue()>avgRatio*threshold){
        logger.info("Avg compression is {}, group {} compression ratio is {}, doesn't start compressing...", avgRatio,
            entry.getKey(), entry.getValue());
        continue;
      }
      logger.info("Avg compression is {}, group {} compression ratio is {}, start compressing...", avgRatio,
          entry.getKey(), entry.getValue());
      storageGroupProcessor.compress();
      compressStorageGroupNum++;
    }
  ...
  /**
   * sync close all compressing storage groups.
   *
   * @throws StorageEngineException StorageEngineException
   */
  public void syncCloseCompress() throws StorageEngineException {
    if (IoTDBDescriptor.getInstance().getConfig().isReadOnly()) {
      throw new StorageEngineException(
          "Current system mode is read only, does not support compress");
    }
    for (StorageGroupProcessor storageGroupProcessor : processorMap.values()) {
      storageGroupProcessor.waitForAllCurrentCompressClosed();
    }
  }
  ...
  /**
   * remove uncompressed tsFiles and add compressed tsfile in one storage group.
  ...
  public void deleteAndloadCompressTsFile(List<TsFileResource> toBeDeletedFile, TsFileResource newTsFileResource)
    ...
    if(newTsFileResource!=null){
      getProcessor(newTsFileResource.getFile().getParentFile().getName())
          .deleteAndloadCompressTsFile(toBeDeletedFile, newTsFileResource);
    }
    else{
      if(toBeDeletedFile==null || toBeDeletedFile.isEmpty()){
        return;
      }
      getProcessor(toBeDeletedFile.get(0).getFile().getParentFile().getName())
          .deleteAndloadCompressTsFile(toBeDeletedFile, newTsFileResource);
    }

  }

  public double getLastIngestRate() {
    return lastIngestRate;
  }

  public double getLastCompressRate() {
    return lastCompressRate;
  }
}
