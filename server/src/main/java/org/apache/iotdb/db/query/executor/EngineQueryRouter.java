...
public class EngineQueryRouter implements IEngineQueryRouter {
  ...
  @Override
  public QueryDataSet queryCompressRatio(QueryExpression queryExpression, QueryContext context)
    ...
    if (queryExpression.hasQueryFilter()) {
      try {
        IExpression optimizedExpression = ExpressionOptimizer.getInstance()
            .optimize(queryExpression.getExpression(), queryExpression.getSelectedSeries());
        queryExpression.setExpression(optimizedExpression);
        CompressRatioQueryExecutor engineExecutor =
            new CompressRatioQueryExecutor(queryExpression);
        if (optimizedExpression.getType() == ExpressionType.GLOBAL_TIME) {
          return engineExecutor.executeWithoutValueFilter(context);
        } else {
          return engineExecutor.executeWithValueFilter(context);
      ...
      CompressRatioQueryExecutor engineExecutor = new CompressRatioQueryExecutor(
          queryExpression);
      try {
        return engineExecutor.executeWithoutValueFilter(context);
      } catch (IOException e) {
