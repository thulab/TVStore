...
public class AggreFuncFactory {
  ...
  public static AggregateFunction getAggrFuncByName(String aggrFuncName, TSDataType dataType)
    ...
    aggrFuncName = aggrFuncName.toLowerCase();
    switch (aggrFuncName) {
      case StatisticConstant.MIN_TIME:
        return new MinTimeAggrFunc(aggrFuncName);
      case StatisticConstant.MAX_TIME:
        return new MaxTimeAggrFunc(aggrFuncName);
      case StatisticConstant.MIN_VALUE:
        return new MinValueAggrFunc(aggrFuncName, dataType);
      case StatisticConstant.MAX_VALUE:
        return new MaxValueAggrFunc(aggrFuncName, dataType);
      case StatisticConstant.COUNT:
        return new CountAggrFunc(aggrFuncName);
      case StatisticConstant.AVG:
        return new AvgAggrFunc(aggrFuncName, dataType);
      case StatisticConstant.FIRST:
        return new FirstAggrFunc(aggrFuncName, dataType);
      case StatisticConstant.SUM:
        return new SumAggrFunc(aggrFuncName, dataType);
      case StatisticConstant.LAST:
        return new LastAggrFunc(aggrFuncName, dataType);
      default:
