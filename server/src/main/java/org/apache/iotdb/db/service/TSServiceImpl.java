...
public class TSServiceImpl implements TSIService.Iface, ServerContext {
  ...
  private List<String> queryColumnsType(List<String> columns, boolean isCompressRatio) throws QueryProcessException {
    if(!isCompressRatio){
      return queryColumnsType(columns);
    }
    List<String> columnTypes = new ArrayList<>();
    for (String column : columns) {
      columnTypes.add(TSDataType.INT32.toString());
    }
    return columnTypes;
  }
  ...
  private TSExecuteStatementResp executeDataQuery(PhysicalPlan plan)
    ...
    if (((QueryPlan) plan).isGroupByDevice()) {
      ...
      switch (plan.getOperatorType()) {
        case QUERY:
          if (((QueryPlan) plan).isQueryCompressRatio()) {
            resp.setColumns(respColumns);
            resp.setDataTypeList(queryColumnsType(respColumns, true));
            return resp;
          }
        case FILL:
          for (Path p : paths) {
