...
public class SeriesReaderWithoutValueFilter implements IPointReader {
  ...
  private IAggregateReader seqResourceIterateReader;
  private IPointReader unseqResourceMergeReader;

  private boolean isEnableRecover = IoTDBDescriptor.getInstance().getConfig().isEnableRecoverRangeQuery();

  public SeriesReaderWithoutValueFilter(IAggregateReader seqResourceIterateReader,
  ...
  private boolean hasNextInBatchDataOrBatchReader() throws IOException {
    ...
    while (seqResourceIterateReader != null && seqResourceIterateReader.hasNext()) {
      PageHeader pageHeader = seqResourceIterateReader.nextPageHeader();
      if(isEnableRecover && pageHeader!=null && pageHeader.getCompressRatio()>1){
        // pageHeader.getCompressRatio()
