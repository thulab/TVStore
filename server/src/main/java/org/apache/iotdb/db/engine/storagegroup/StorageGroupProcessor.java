...
public class StorageGroupProcessor {
  ...
  /**
   * This is the modification file of the result of the current compress. Because the compressed
   * file may be invisible at this moment, without this, deletion/update during merge could be lost.
   */
  private ModificationFile compressingModification;
  private static final String COMPRESSING_MODIFICATION_FILE_NAME = "compress.mods";
  private volatile boolean isCompressing = false;
  private long compressStartTime;
  private AtomicLong writePointNum = new AtomicLong(0);

  /**
   * compressLock is to be used in the compress process. Concurrent queries, deletions，merges and
   * compresses may result in losing some deletion in the compressed new file, so a lock is necessary.
   */
  private ReentrantReadWriteLock compressLock = new ReentrantReadWriteLock();
  /**
   * when the data in a storage group is older than compressTTL, it is considered unimportant and
   * will be eventually compressed.
   */
  private long compressTTL = IoTDBDescriptor.getInstance().getConfig().getCompressTTL();

  /**
   * closeCompressCondition is used to wait for all currently compressing task to be done.
   */
  private final Object closeCompressCondition = new Object();
  ...
  private void recover() throws StorageGroupProcessorException {
    ...
    try {
      ...
      File compressingMods = SystemFileFactory.INSTANCE.getFile(storageGroupSysDir,
          COMPRESSING_MODIFICATION_FILE_NAME);
      if (compressingMods.exists()) {
        compressingModification = new ModificationFile(compressingMods.getPath());
      }
      ...
      if (!IoTDBDescriptor.getInstance().getConfig().isContinueCompressAfterReboot()) {
        compressingMods.delete();
      }
  ...
  public Integer[] insertBatch(BatchInsertPlan batchInsertPlan) throws QueryProcessException {
    ...
    try {
      ...
      writePointNum.addAndGet(batchInsertPlan.getRowCount());
      return results;
    } finally {
      writeUnlock();
    }
  ...
  public QueryDataSource query(String deviceId, String measurementId, QueryContext context,
      JobFileManager filePathsManager, Filter filter) {
    insertLock.readLock().lock();
    mergeLock.readLock().lock();
    compressLock.readLock().lock();
    ...
    try {
      List<TsFileResource> seqResources = getFileReSourceListForQuery(sequenceFileList,
          deviceId, measurementId, context, filter);
      ...
      compressLock.readLock().unlock();
    }
  }

  public QueryDataSource query(String deviceId, String measurementId, QueryContext context,
      JobFileManager filePathsManager) {
    return query(deviceId, measurementId, context, filePathsManager, null);
  }
  ...
  private List<TsFileResource> getFileReSourceListForQuery(List<TsFileResource> tsFileResources,
      String deviceId, String measurementId, QueryContext context, Filter filter) {
    ...
    for (TsFileResource tsFileResource : tsFileResources) {
      ...
      try {
        if (tsFileResource.isClosed()) {
          ...
          if(filter!=null&&!filter.satisfyStartEndTime(tsFileResource.getStartTimeMap().get(deviceId),Long.MAX_VALUE)){
            continue;
          }
  ...
  public void delete(String deviceId, String measurementId, long timestamp) throws IOException {
    ...
    try {
      ...
      if (compressingModification != null) {
        compressingModification.write(deletion);
        updatedModFiles.add(compressingModification);
      }
  ...
  public boolean isCompressing(){
    writeLock();
    logger.debug("[After write lock]{} starts to compress {} tsfile.",storageGroupName, sequenceFileList.size());
    try {
      if (isCompressing) {
        if (logger.isInfoEnabled()) {
          logger
              .info("{} last compress is ongoing, currently consumed time: {}ms", storageGroupName,
                  (System.currentTimeMillis() - compressStartTime));
        }
        return true;
      }
      else{
  ...
  public double calCompressRatio() {
    writeLock();
    try {
      Pair<Integer, Integer> actualVirtuaPagePair = MergeUtils.collectActualVirtualPageNumPair(sequenceFileList);
      return (1 + actualVirtuaPagePair.right) * 1.0 / (actualVirtuaPagePair.left + 1);
    } finally {
      writeUnlock();
    }
  }
  public void compress() {
    logger.debug("[Before write lock]{} starts to compress {} tsfile.",storageGroupName, sequenceFileList.size());
    writeLock();
    logger.debug("[After write lock]{} starts to compress {} tsfile.",storageGroupName, sequenceFileList.size());
    try {
      if (isCompressing) {
        if (logger.isInfoEnabled()) {
          logger.info("{} last compress is ongoing, currently consumed time: {}ms", storageGroupName,
              (System.currentTimeMillis() - compressStartTime));
        }
        return ;
      }
      ...
      if (sequenceFileList.isEmpty()) {
        logger.info("{} no files to be compressed", storageGroupName);
        isCompressing = false;
        return;
      }
      logger.info("Thread {}, group {} starts to compress {} tsfile.", Thread.currentThread().getName(), storageGroupName, sequenceFileList.size());

      //FIXME 改成距当前时间，目前测试取设备的最新时间
      //long timeHigherBound = System.currentTimeMillis() - compressTTL;
      long curTime = Collections.max(latestTimeForEachDevice.values());
      long timeHigherBound = curTime - compressTTL;
      CompressResource mergeResource = new CompressResource(sequenceFileList, timeHigherBound);

      try {
//        List[] mergeFiles = fileSelector.select();
        if (mergeResource.getSeqFiles().size() == 0) {
          logger.info("{} cannot select compress candidate files.", storageGroupName);
          return;
        }
//        // avoid pending tasks holds the metadata and streams
        mergeResource.clear();
        ...
        for (TsFileResource tsFileResource : mergeResource.getSeqFiles()) {
          tsFileResource.setCompressing(true);
          logger.info("{} set file {} compress.", storageGroupName, tsFileResource.getFile().getAbsolutePath());
        }
        int concurrentCompressSeriesNum = IoTDBDescriptor.getInstance().getConfig().getCompressChunkSubthreadNum();
        CompressTask compressTask = new CompressTask(mergeResource, storageGroupSysDir.getPath(),
            this::compressEndAction, taskName,  concurrentCompressSeriesNum,
            storageGroupName);
        compressingModification = new ModificationFile(
            storageGroupSysDir + File.separator + COMPRESSING_MODIFICATION_FILE_NAME);
        CompressManager.getINSTANCE().submitMainTask(compressTask);
        if (logger.isInfoEnabled()) {
          logger.info("{} submits a compress task {}, compressing {} seqFiles",
              storageGroupName, taskName, sequenceFileList.size());
        }
        compressStartTime = System.currentTimeMillis();

      } catch ( IOException e) {
        logger.error("{} cannot select file for compress", storageGroupName, e);
      }
    } finally {
      writeUnlock();
    }
  ...
  private void updateCompressModification(TsFileResource seqFile) {
    seqFile.getWriteQueryLock().writeLock().lock();
    try {
      // remove old modifications and write modifications generated during merge
      seqFile.removeModFile();
      if (compressingModification != null) {
        for (Modification modification : compressingModification.getModifications()) {
          seqFile.getModFile().write(modification);
        }
      }
    } catch (IOException e) {
      logger.error("{} cannot clean the ModificationFile of {} after compress", storageGroupName,
          seqFile.getFile(), e);
    } finally {
      seqFile.getWriteQueryLock().writeLock().unlock();
    }
  ...
  private void removeCompressingModification() {
    try {
      if (compressingModification != null) {
        compressingModification.remove();
        compressingModification = null;
      }
    } catch (IOException e) {
      logger.error("{} cannot remove compressing modification ", storageGroupName, e);
    }
  }

  protected void compressEndAction(List<TsFileResource> seqFiles, Boolean isDeleteUncompressTsFile,
      File mergeLog) {
    logger.info("{} a compress task is ending...", storageGroupName);

    for (int i = 0; i < seqFiles.size(); i++) {
      TsFileResource seqFile = seqFiles.get(i);
      compressLock.writeLock().lock();
      try {
        updateCompressModification(seqFile);
        if (i == seqFiles.size() - 1) {
          removeCompressingModification();
          isCompressing = false;
          mergeLog.delete();
        }
      } finally {
    ...
    logger.info("{} a compress task ends", storageGroupName);
  }

  /**
   * remove uncompressed tsFiles and add compressed tsfile.
   * @param toBeDeletedFile remove uncompressed tsFiles
   * @param newTsFileResource add compressed tsfile
   * @throws IOException
   * @throws StorageEngineException
   */
  public void deleteAndloadCompressTsFile(List<TsFileResource> toBeDeletedFile, TsFileResource newTsFileResource)
    ...
    for (TsFileResource seqFile : toBeDeletedFile) {
      ...
      try {
        ...
        StorageEngine.getInstance().deleteTsfile(seqFile.getFile());
        logger.info("[COMPRESS] storage group {} delete file {}.", storageGroupName, seqFile.getFile().getAbsolutePath());
      } finally {
        seqFile.getWriteQueryLock().writeLock().unlock();
      }
    }
  ...
  /**
   * This method will be blocked until all compress task are ended.
   */
  public void waitForAllCurrentCompressClosed() {
    long st = System.currentTimeMillis();
    synchronized (closeCompressCondition) {
      try {
        while (isCompressing) {
          closeCompressCondition.wait();
        }
      } catch (InterruptedException e) {
        logger.error("closeCompressCondition error occurs while waiting for ending the storage "
            + "group {}", storageGroupName, e);
      }
    }
    logger.info("WAIT FOR COMPRESS END in storage group {} cost {}s.", storageGroupName, (System.currentTimeMillis()-st)/1000);
  }
  ...
  private void loadTsFileByType(LoadTsFileType type, File syncedTsFile,
    ...
    File syncedTsFileResource = new File(syncedTsFile.getAbsolutePath() + TsFileResource.RESOURCE_SUFFIX);
    File targetTsFileResource = new File(targetFile.getAbsolutePath() + TsFileResource.RESOURCE_SUFFIX);

    if(syncedTsFileResource.exists() && !targetTsFileResource.exists()){
      try {
        FileUtils.moveFile(syncedTsFileResource, targetTsFileResource);
      } catch (IOException e) {
        throw new TsFileProcessorException(String.format(
  ...
  public long updateIngestSpeed(){
    long lastNum = writePointNum.get();
    writePointNum.set(0);
    return lastNum;
  }
