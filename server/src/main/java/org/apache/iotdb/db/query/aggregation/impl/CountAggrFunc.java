...
public class CountAggrFunc extends AggregateFunction {
  ...
  public CountAggrFunc(String funName) {
    super(funName, TSDataType.INT64);
  }
  ...
  public void calculateValueFromPageHeader(PageHeader pageHeader, double ratio)
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("PageHeader>>>>>>>>>>>>num of rows:{}, minTimeStamp:{}, maxTimeStamp:{}, ratio:{}",
          pageHeader.getNumOfValues(), pageHeader.getMinTimestamp(), pageHeader.getMaxTimestamp(), ratio);
    }
    long preValue = resultData.getLongRet();
    preValue += pageHeader.getNumOfValues()*ratio;
    resultData.setLongRet(preValue);
  }
  ...
  public void calculateValueFromPageData(PageHeader pageHeader, BatchData dataInThisPage, IPointReader unsequenceReader)
      throws IOException {
    int cnt = calculateValueFromPageData(dataInThisPage, unsequenceReader, false, 0);
    if(pageHeader!=null && pageHeader.getCompressRatio()>1){
      cnt = cnt*pageHeader.getCompressRatio();
    }
    long preValue = resultData.getLongRet();
    preValue += cnt;
    resultData.setLongRet(preValue);
  ...
  public void calculateValueFromPageData(BatchData dataInThisPage, IPointReader unsequenceReader,
    ...
    int cnt = calculateValueFromPageData(dataInThisPage, unsequenceReader, true, bound);
    long preValue = resultData.getLongRet();
    preValue += cnt;
    resultData.setLongRet(preValue);
  }

  private int calculateValueFromPageData(BatchData dataInThisPage, IPointReader unsequenceReader,
